...
let itensVela = document.querySelector(".itens-vela");
let id = 0;
for (const p of produtos) {
    ...
    <div class="">
        ...
        <div class="info">
            <h3>${p.nome}</h3>
            <h4>Pequena <span>R$${p.fatias6}</span></h4>
            <h4>Media <span>R$${p.fatias8}</span></h4>
            <h4>Grande <span>R$${p.fatias12}</span></h4>
...
function atualizarTabela() {
    ...
    for (const p of produtos) {
        if (p.quantidade > 0) {
            ...
            <tr>
                <td>${p.nome}</td>
                <td>${p.quantidade}x${p.fatias8}=${p.quantidade*p.fatias8}( vela)</td>
                <td>
